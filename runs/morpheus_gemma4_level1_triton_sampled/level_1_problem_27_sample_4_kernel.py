...
def selu_kernel(
    ...
    # SELU: lambda * x if x > 0 else lambda * (alpha * exp(x) - alpha)
    out = tl.where(x > 0, LAMBDA * x, LAMBDA * (ALPHA * tl.exp(x) - ALPHA))
...
class ModelNew(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Applies SELU activation to the input tensor using a Triton kernel.
