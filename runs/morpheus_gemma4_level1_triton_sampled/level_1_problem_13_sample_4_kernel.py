...
def matmul_kernel(
    ...
    # Compute offsets for C and store the result
    c_offsets = rm[:, None] * stride_cm + rn[None, :] * stride_cn
    c = tl.load(c_ptr + c_offsets, mask=(rm[:, None] < M) & (rn[None, :] < N), other=0.0)
    c += accumulator
    tl.store(c_ptr + c_offsets, c, mask=(rm[:, None] < M) & (rn[None, :] < N))
