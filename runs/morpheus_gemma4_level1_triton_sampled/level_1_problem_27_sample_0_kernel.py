...
def selu_kernel(
    ...
    # Constants for SELU
    ALPHA = 1.6732632423543772848170310530612246803
    LAMBDA = 1.0507009873554804934193349852946119472
    ...
    # SELU formula: lambda * x if x > 0 else lambda * alpha * (exp(x) - 1)
    # We can factor out lambda: lambda * (x if x > 0 else alpha * (exp(x) - 1))
...
def triton_selu(x: torch.Tensor):
    ...
    # Block size for element-wise operations
    BLOCK_SIZE = 1024
