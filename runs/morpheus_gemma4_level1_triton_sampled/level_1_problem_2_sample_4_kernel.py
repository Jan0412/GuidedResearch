...
def matmul_kernel(
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        # Masking is used to handle cases where M, N, or K are not multiples of the block size
        a = tl.load(a_ptrs, mask=(rm[:, None] < M) & (rk[None, :] < K), other=0.0)
        b = tl.load(b_ptrs, mask=(rk[:, None] < K) & (rn[None, :] < N), other=0.0)

        # Perform matrix multiplication for the tiles
...
def triton_matmul(a: torch.Tensor, b: torch.Tensor):
    ...
    # Grid definition: one program per block of output C
...
class ModelNew(nn.Module):
    ...
    def forward(self, A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
        ...
        """
        # Ensure inputs are on GPU as Triton kernels require CUDA tensors
        if not A.is_cuda:
            A = A.cuda()
        if not B.is_cuda:
            B = B.cuda()
