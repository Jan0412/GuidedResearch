...
def matmul_kernel(
    ...
    """
    Triton kernel for computing C = A * B^T
    ...
    # L2 cache optimization: group blocks of M to improve reuse of blocks of B
    ...
    # B block: (BLOCK_SIZE_K, BLOCK_SIZE_N) - since we are doing A * B^T, we access B as (N, K)
    # We want the k-dimension to be the rows of the block for tl.dot
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        b = tl.load(b_ptrs, mask=(offs_bn[None, :] < N) & (offs_k[:, None] < K - k * BLOCK_SIZE_K), other=0.0)
...
class ModelNew(nn.Module):
    ...
    def forward(self, A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
        ...
        """
        # Ensure inputs are float32 for the kernel as requested
