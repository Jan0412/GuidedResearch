...
def triton_scalar_mul(A: torch.Tensor, s: float):
    ...
    """
    assert A.is_cuda, "Tensor A must be on CUDA."
    
    # Ensure the tensor is contiguous for linear indexing
    A = A.contiguous()
    ...
    n_elements = A.numel()
    # Use a large block size for element-wise operations to maximize throughput
    BLOCK_SIZE = 1024 
    
    # Grid is 1D over the total number of elements
