...
def matmul_kernel(
    ...
    """
    # Map program ID to the block of the output matrix C
    pid = tl.program_id(0)
    num_pid_m = tl.cdiv(M, BLOCK_M)
    num_pid_n = tl.cdiv(N, BLOCK_N)
    
    # L2 cache optimization: group programs to improve memory reuse
    ...
    # Pointers to the current blocks of A and B
    # A: (M, K), B: (K, N)
    ...
    for k in range(0, tl.cdiv(K, BLOCK_K)):
        # Load blocks with masking to handle non-multiple dimensions
