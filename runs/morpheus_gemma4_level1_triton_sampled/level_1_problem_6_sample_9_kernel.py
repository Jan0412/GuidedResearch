...
def matmul_kernel(
    ...
    Optimized for large K dimension.
    """
    ...
    # L2 cache optimization: reorder the blocks to be processed
    group_id = pid_n // (BLOCK_SIZE_N // GROUP_SIZE_M) if GROUP_SIZE_M > 0 else 0
    first_pid_m = group_id * GROUP_SIZE_M
    group_size_m = min(num_pid_m - first_pid_m, GROUP_SIZE_M)
    pid_m = first_pid_m + (pid % group_size_m) if GROUP_SIZE_M > 0 else pid_m
