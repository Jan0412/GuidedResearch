...
def triton_hardtanh(x: torch.Tensor, min_val: float = -1.0, max_val: float = 1.0):
    ...
    Args:
        ...
        min_val (float): Minimum value for clamping (default: -1.0)
        max_val (float): Maximum value for clamping (default: 1.0)
    ...
    hardtanh_kernel[grid](x, out, n_elements, min_val=min_val, max_val=max_val, BLOCK_SIZE=BLOCK_SIZE)
    return out


class ModelNew(nn.Module):
    """
    Optimized model that performs a HardTanh activation using Triton kernel.
    """
    def __init__(self):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Applies HardTanh activation to the input tensor using optimized Triton kernel.
