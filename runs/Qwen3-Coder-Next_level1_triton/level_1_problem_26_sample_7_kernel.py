...
def gelu_kernel(
    ...
    # Compute GELU: x * 0.5 * (1.0 + erf(x / sqrt(2)))
    # Using the approximation with erf for better numerical stability
    sqrt_2_inv = 1.0 / tl.sqrt(2.0)
    erf_arg = x * sqrt_2_inv
...
class ModelNew(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Applies GELU activation to the input tensor using optimized Triton kernel.
