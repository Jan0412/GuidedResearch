...
def swish_kernel(
    ...
    # Using stable sigmoid computation to avoid overflow
    sig = tl.where(x >= 0, 
                   1.0 / (1.0 + tl.exp(-x)), 
...
def triton_swish(x: torch.Tensor):
    """
    This function wraps the Triton kernel call for Swish activation. It:
...
class ModelNew(nn.Module):
    """
    Optimized model that performs Swish activation using a custom Triton kernel.
    """
    def __init__(self):
