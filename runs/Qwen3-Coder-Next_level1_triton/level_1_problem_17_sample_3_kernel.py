...
def matmul_kernel(
    ...
    for k in range(0, K, BLOCK_SIZE_K):
        # Load A block
        a = tl.load(
            A_ptr + (off_m[:, None] * stride_am + (k + off_k[None, :]) * stride_ak),
            mask=amask[:, None] & anmask[None, :],
            other=0.0
        )
        
        # Load B block (note: B is transposed in the original operation, so we use B directly)
        b = tl.load(
            B_ptr + ((k + off_k[:, None]) * stride_bk + off_n[None, :] * stride_bn),
            mask=bnmask[:, None] & bnmask2[None, :],
            other=0.0
        )
...
def triton_matmul(A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        ...
        B: Input tensor of shape (N, K) (will be used as B.T in matmul)
    ...
    assert A.shape[1] == B.shape[1], f"Inner dimensions must match: A.shape={A.shape}, B.shape={B.shape}"
