...
def selu_kernel(
    ...
    # SELU activation: lambda * (alpha * exp(x) - alpha) if x <= 0 else lambda * x
    ...
    # Compute SELU: f(x) = lambda * (alpha * exp(x) - alpha) if x <= 0 else lambda * x
    selu_out = tl.where(
        x <= 0,
        lambda_val * (alpha * tl.exp(x) - alpha),
        lambda_val * x
    )
