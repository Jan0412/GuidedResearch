...
def hardsigmoid_kernel(
    ...
    # This is equivalent to torch.nn.functional.hardsigmoid
    temp = (x + 3.0) / 6.0
...
def triton_hardsigmoid(x: torch.Tensor):
    """
    This function wraps the Triton kernel call for HardSigmoid.
    It:
...
class ModelNew(nn.Module):
    """
    Optimized model that performs a HardSigmoid activation using Triton kernel.
    """
    def __init__(self):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Applies HardSigmoid activation to the input tensor using a Triton kernel.
