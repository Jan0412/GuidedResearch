...
def hinge_loss_kernel(
    ...
    # Apply clamp with min=0 (ReLU)
    clamped = tl.maximum(temp, 0.0)
...
def mean_reduction_kernel(
    ...
    # Reduce to sum
    reduced_sum = tl.sum(input_vals, axis=0)
    
    # Store partial sum
    tl.store(output_ptr + tl.program_id(0), reduced_sum, mask=tl.program_id(0) < (n_elements + BLOCK_SIZE - 1) // BLOCK_SIZE)
