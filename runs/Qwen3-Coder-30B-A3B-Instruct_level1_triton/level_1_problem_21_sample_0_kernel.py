...
def sigmoid_kernel(
    ...
    # Using the stable implementation to avoid overflow
    y = 1.0 / (1.0 + tl.exp(-x))
...
def triton_sigmoid(x: torch.Tensor):
    """
    Triton kernel wrapper for sigmoid activation
    """
...
class ModelNew(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Applies Sigmoid activation using Triton kernel for better performance
