...
def triton_matmul_torch(a: torch.Tensor, b: torch.Tensor):
    """
    Custom Triton implementation of matrix multiplication for A^T @ B
    """
    assert a.dim() == 2 and b.dim() == 2, "Both tensors must be 2D"
    assert a.shape[0] == b.shape[0], "Matrix dimensions incompatible for multiplication"
    
    # Transpose A since we're computing A^T @ B
    a_t = a.T
    M, K = a_t.shape
    K_, N = b.shape
    assert K == K_, f"Matrix dimensions incompatible: {a_t.shape} @ {b.shape}"
    ...
    grid = (num_blocks_m * num_blocks_n, 1, 1)
...
class ModelNew(nn.Module):
    ...
    def forward(self, A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
        """
        Performs matrix multiplication: A^T @ B using Triton kernel
