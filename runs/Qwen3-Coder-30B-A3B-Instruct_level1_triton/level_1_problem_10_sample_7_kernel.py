...
def triton_matmul(a: torch.Tensor, b: torch.Tensor):
    ...
    assert a.shape[2] == b.shape[0], "Inner dimensions must match for matrix multiplication"
    ...
    m, n, k = a.shape
    _, l = b.shape
    c = torch.empty(m, n, l, dtype=torch.float32, device='cuda')
    ...
    grid = (
        triton.cdiv(n, BLOCK_N) * triton.cdiv(l, BLOCK_M),
        m
    )
...
class ModelNew(nn.Module):
    """
    Optimized version using custom Triton kernel for 3D tensor-matrix multiplication.
    """
    def __init__(self):
