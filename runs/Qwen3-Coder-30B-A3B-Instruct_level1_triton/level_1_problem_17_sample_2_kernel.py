...
def matmul_kernel(
    ...
    # Compute offsets for the block of A and B
...
def triton_matmul(A: torch.Tensor, B: torch.Tensor):
    ...
    """
    assert A.dim() == 2 and B.dim() == 2, "Both matrices must be 2D"
    ...
    matmul_kernel[grid](
        ...
        ACTIVATION="leaky_relu"  # This is not used but kept for compatibility
    )
...
class ModelNew(nn.Module):
    ...
    def forward(self, A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            ...
            B: Input tensor of shape (N, K) - note: we transpose this internally
            
        Returns:
            Output tensor of shape (M, N).
        """
        # Transpose B to make it (K, N) for proper matrix multiplication
        B_T = B.T
