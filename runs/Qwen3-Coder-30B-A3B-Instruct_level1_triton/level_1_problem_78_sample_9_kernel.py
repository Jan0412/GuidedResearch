import torch
import torch.nn as nn
import torch.nn.functional as F
import triton
import triton.language as tl
import math

@triton.jit
def conv_transpose2d_kernel(
    input_ptr,
    weight_ptr,
    output_ptr,
    bias_ptr,
    batch_size,
    in_channels,
    out_channels,
    input_height,
    input_width,
    output_height,
    output_width,
    kernel_height,
    kernel_width,
    stride_h,
    stride_w,
    pad_h,
    pad_w,
    BLOCK_SIZE: tl.constexpr,
    CHANNELS_PER_BLOCK: tl.constexpr,
    OUTPUT_H_PER_BLOCK: tl.constexpr,
    OUTPUT_W_PER_BLOCK: tl.constexpr
):
    # Get thread indices
    batch_idx = tl.program_id(0)
    channel_block = tl.program_id(1)
    output_h_block = tl.program_id(2)
    output_w_block = tl.program_id(3)
    
    # Calculate output position
    out_h_start = output_h_block * OUTPUT_H_PER_BLOCK
    out_w_start = output_w_block * OUTPUT_W_PER_BLOCK
    
    # Shared memory for input tile
    shared_input = tl.shared_memory(dtype=tl.float32, shape=(OUTPUT_H_PER_BLOCK + 2 * pad_h, OUTPUT_W_PER_BLOCK + 2 * pad_w))
    
    # Initialize accumulator
    acc = tl.zeros((OUTPUT_H_PER_BLOCK, OUTPUT_W_PER_BLOCK), dtype=tl.float32)
    
    # Loop over input channels
    for c in range(channel_block * CHANNELS_PER_BLOCK, (channel_block + 1) * CHANNELS_PER_BLOCK):
        if c >= in_channels:
            break
            
        # Load input data for this channel
        input_offset = batch_idx * in_channels * input_height * input_width + c * input_height * input_width
        input_ptr_local = input_ptr + input_offset
        
        # Load weight for this channel
        weight_offset = c * out_channels * kernel_height * kernel_width
        weight_ptr_local = weight_ptr + weight_offset
        
        # Process each kernel position
        for kh in range(kernel_height):
            for kw in range(kernel_width):
                # Calculate input position
                input_h_start = out_h_start * stride_h - pad_h + kh
                input_w_start = out_w_start * stride_w - pad_w + kw
                
                # Check bounds
                if input_h_start >= 0 and input_h_start < input_height and input_w_start >= 0 and input_w_start < input_width:
                    # Load input value
                    input_val = tl.load(input_ptr_local + input_h_start * input_width + input_w_start, mask=True)
                    
                    # Load weight value
                    weight_val = tl.load(weight_ptr_local + kh * kernel_width + kw, mask=True)
                    
                    # Accumulate
                    acc += input_val * weight_val
    
    # Apply bias if exists
    if bias_ptr is not None:
        bias_offset = channel_block * CHANNELS_PER_BLOCK
        bias_vals = tl.load(bias_ptr + bias_offset, mask=(channel_block * CHANNELS_PER_BLOCK + tl.arange(0, CHANNELS_PER_BLOCK)) < out_channels)
        for i in range(OUTPUT_H_PER_BLOCK):
            for j in range(OUTPUT_W_PER_BLOCK):
                if out_h_start + i < output_height and out_w_start + j < output_width:
                    bias_idx = channel_block * CHANNELS_PER_BLOCK + (out_w_start + j) % out_channels
                    acc[i, j] += bias_vals[0]  # Simplified bias application
    
    # Store results
    output_offset = batch_idx * out_channels * output_height * output_width + channel_block * OUTPUT_H_PER_BLOCK * OUTPUT_W_PER_BLOCK
    output_ptr_local = output_ptr + output_offset
    
    for i in range(OUTPUT_H_PER_BLOCK):
        for j in range(OUTPUT_W_PER_BLOCK):
            if out_h_start + i < output_height and out_w_start + j < output_width:
                tl.store(output_ptr_local + (out_h_start + i) * output_width + (out_w_start + j), acc[i, j])

def triton_conv_transpose2d(input_tensor, weight, bias, stride, padding):
    """
    Triton implementation of ConvTranspose2d
    """
    batch_size, in_channels, input_height, input_width = input_tensor.shape
    out_channels, _, kernel_height, kernel_width = weight.shape
    
    # Calculate output dimensions
    output_height = (input_height - 1) * stride[0] - 2 * padding[0] + kernel_height
    output_width = (input_width - 1) * stride[1] - 2 * padding[1] + kernel_width
    
    # Create output tensor
    output = torch.empty(batch_size, out_channels, output_height, output_width, device=input_tensor.device, dtype=torch.float32)
    
    # Define block sizes
    BLOCK_SIZE = 128
    CHANNELS_PER_BLOCK = 16
    OUTPUT_H_PER_BLOCK = 16
    OUTPUT_W_PER_BLOCK = 16
    
    # Grid configuration
    grid = (
        batch_size,  # Batch dimension
        (out_channels + CHANNELS_PER_BLOCK - 1) // CHANNELS_PER_BLOCK,  # Channel blocks
        (output_height + OUTPUT_H_PER_BLOCK - 1) // OUTPUT_H_PER_BLOCK,  # Output H blocks
        (output_width + OUTPUT_W_PER_BLOCK - 1) // OUTPUT_W_PER_BLOCK   # Output W blocks
    )
    
    # Launch kernel
    conv_transpose2d_kernel[grid](
        input_tensor,
        weight,
        output,
        bias,
        batch_size,
        in_channels,
        out_channels,
        input_height,
        input_width,
        output_height,
        output_width,
        kernel_height,
        kernel_width,
        stride[0],
        stride[1],
        padding[0],
        padding[1],
        BLOCK_SIZE,
        CHANNELS_PER_BLOCK,
        OUTPUT_H_PER_BLOCK,
        OUTPUT_W_PER_BLOCK
    )
    
    return output

class ModelNew(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, kernel_size: tuple, stride: tuple = (1, 1), padding: tuple = (0, 0), bias: bool = False):
        super(ModelNew, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.kernel_size = kernel_size
        self.stride = stride
        self.padding = padding
        self.bias = bias
        
        # Initialize weights and bias
        self.weight = nn.Parameter(torch.randn(out_channels, in_channels, kernel_size[0], kernel_size[1]))
        if bias:
            self.bias = nn.Parameter(torch.randn(out_channels))
        else:
            self.register_parameter('bias', None)
            
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Performs the 2D transposed convolution using Triton kernel.
        """
        # Ensure inputs are contiguous
        x = x.contiguous()
        
        # Use Triton kernel for computation
        return triton_conv_transpose2d(x, self.weight, self.bias, self.stride, self.padding)