...
def tril_kernel(
    ...
    # Load flattened input
    input_flat = tl.load(input_ptr + offsets, mask=mask, other=0.0)
    ...
    condition = row >= col
    result = tl.where(condition, input_flat, 0.0)
...
def triton_tril(X: torch.Tensor):
    """Custom Triton kernel for applying lower triangular mask"""
...
class ModelNew(nn.Module):
    """
    Optimized model using Triton kernels for matrix multiplication and triangular masking.
    """
    def __init__(self):
    ...
    def forward(self, A, B):
        ...
        Uses Triton kernels for both operations.
        ...
        # Apply lower triangular mask using Triton kernel
        result = triton_tril(C)
