...
class ModelNew(nn.Module):
    ...
    def forward(self, A, B):
        ...
        """
        # Reshape A to 2D for processing with our Triton kernel
        N, M, K = A.shape
        K_, L = B.shape
        ...
        # Reshape A to (N*M, K) for efficient processing
        A_reshaped = A.view(-1, K)
        
        # Perform matrix multiplication using Triton kernel
        result_reshaped = triton_matmul(A_reshaped, B)
        
        # Reshape back to original output shape (N, M, L)
        return result_reshaped.view(N, M, L)