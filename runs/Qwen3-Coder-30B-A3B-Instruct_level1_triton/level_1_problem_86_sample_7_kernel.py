...
def depthwise_conv2d_kernel(
    ...
    # Check bounds for channels
    channel_mask = global_channel_idx < in_channels
    
    # Shared memory for input tile
    input_tile = tl.shared(1, BLOCK_SIZE, BLOCK_SIZE)
    ...
    for output_x in range(output_w):
        ...
        acc = tl.zeros((CHANNELS_PER_BLOCK, 1), dtype=tl.float32)
        ...
        for ky in range(kernel_h):
            for kx in range(kernel_w):
                ...
                valid_y = (input_y >= 0) & (input_y < height)
                valid_x = (input_x >= 0) & (input_x < width)
                ...
                input_val = tl.load(input_ptr + 
                                  ...
                                  mask=(valid & channel_mask), other=0.0)
        ...
        # Store output
        output_offset = batch_idx * (in_channels * output_h * output_w) + \
