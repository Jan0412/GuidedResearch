...
def l2_norm_kernel(
    ...
    for i in range(0, n_cols, BLOCK_SIZE):
        ...
        # Load elements
        x_vals = tl.load(x_ptr + x_row_start + off, mask=mask, other=0.0)
    ...
    reciprocal_norm = tl.where(l2_norm > 0.0, 1.0 / l2_norm, 0.0)
    ...
    for i in range(0, n_cols, BLOCK_SIZE):
        ...
        # Normalize and store
        normalized_vals = x_vals * reciprocal_norm
        tl.store(out_ptr + out_row_start + off, normalized_vals, mask=mask)
