...
def elu_kernel(
    ...
    # Using torch's ELU formula: where x < 0, return alpha * (exp(x) - 1), else return x
    # We compute exp(x) using tl.exp, then apply the formula
    elu_result = tl.where(x < 0, alpha * (tl.exp(x) - 1.0), x)
