...
def matmul_kernel(
    ...
    for k in range(0, K, BLOCK_SIZE_K):
        # Load tiles
        a_tile = tl.load(a_ptrs, mask=offs_k[None, :] < K - k, other=0.0)
        b_tile = tl.load(b_ptrs, mask=offs_k[:, None] < K - k, other=0.0)
...
def triton_matmul(a, b):
    ...
    # Transpose inputs as per original model requirements
    a_t = a.T
    b_t = b.T
