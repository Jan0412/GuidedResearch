...
def triton_mse(predictions: torch.Tensor, targets: torch.Tensor):
    ...
    # Allocate intermediate tensor for squared differences
    ...
    # Second kernel: reduce to sum
    sum_tensor = torch.tensor(0.0, device=predictions.device, dtype=torch.float32)
    grid2 = lambda meta: (1,)
    sum_kernel[grid2](squared_diff, sum_tensor, n_elements, BLOCK_SIZE=BLOCK_SIZE)
    
    # Third kernel: compute mean
    mean_result = torch.empty(1, device=predictions.device, dtype=torch.float32)
    grid3 = lambda meta: (1,)
    mean_kernel[grid3](sum_tensor, mean_result, n_elements, BLOCK_SIZE=BLOCK_SIZE)
    
    return mean_result.item()
