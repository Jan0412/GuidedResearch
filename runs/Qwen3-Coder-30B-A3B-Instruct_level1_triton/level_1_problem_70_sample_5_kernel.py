...
def conv_transpose3d_kernel(
    ...
    output_d = tl.program_id(3)
    output_h = tl.program_id(4)
    output_w = tl.program_id(5)
    
    # Shared memory for input tile
    shared_input = tl.shared_ptr(input_ptr, shape=(BLOCK_SIZE, BLOCK_SIZE, BLOCK_SIZE), dtype=tl.float32)
    
    # Calculate which group this thread belongs to
    group_start = group_idx * GROUPS_PER_BLOCK
            ...
            for kw in range(kernel_width):
                ...
                if (input_d >= 0 and input_d < input_depth and
                    ...
                    input_idx = (batch_idx * (in_channels * input_depth * input_height * input_width) +
                               (group_start * (input_depth * input_height * input_width)) +
                               (input_d * input_height * input_width) +
                               (input_h * input_width) +
                               input_w)
                    
                    # Calculate weight index
                    weight_idx = (channel_idx * (groups * kernel_depth * kernel_height * kernel_width) +
                                (group_start * (kernel_depth * kernel_height * kernel_width)) +
...
def triton_conv_transpose3d(input_tensor, weight, bias=None, stride=(1,1,1), padding=(0,0,0), 
    ...
    grid = (
        batch_size,
        math.ceil(groups / 4),  # Assuming 4 groups per block
        math.ceil(out_channels / 4),  # Assuming 4 channels per block
        output_depth,
        output_height,
        output_width
    )
