...
def _round_of(leaf_dir: str) -> int | None:
    """Lint-loop round from a leaf path, or None for a shard root (final kernels)."""
    parent, tail = os.path.split(leaf_dir.rstrip("/"))
    if os.path.basename(parent) == "rounds" and tail.startswith("round_"):
        return int(tail.split("_")[-1])
    return None


def _generator_map(run_dirs: list[str]) -> dict[str, str]:
    """``{absolute run root: basename}`` -- the grouping unit a leaf belongs to.

    ``run_name`` from ``expand_run_dirs`` is the *leaf* (``<run>__shard_00__round1``) and
    must stay unique, because it is the join key the scorer and policy table use. But a
    "problem" for ranking is one generator's candidates, pooled across its shards and
    rounds -- so the grouping needs the run root, carried separately as ``generator``.
    """
    out = {}
    for r in run_dirs:
        root = os.path.abspath(_resolve(r))
...
def build_eval_table(
    ...
    # A run expands to shard_NN/ and, with `rounds`, shard_NN/rounds/round_R -- the same
    # walk the training build does, so run_name here == run_name there. Omit `rounds` for
    # the kernel each sample finished on; pass a subset to bin one round.
    gen_of = _generator_map(run_dirs)
    leaves = expand_run_dirs(run_dirs, levels or [None] * len(run_dirs), rounds)
    print(f"[expand] {len(run_dirs)} run(s) -> {len(leaves)} leaf dir(s) "
          f"(rounds: {rounds if rounds is not None else 'final only'})")
    ...
    with open(out_path, "w") as out_f:
        for leaf_dir, run_name, level_override in leaves:
            ...
            rnd = _round_of(leaf_dir)
            eval_results = load_eval_results(leaf_dir)
            if eval_results is None:
                continue

            n_run = n_compiled = n_correct = 0
            for path, level, pid, sid in iter_run_kernels(leaf_dir):
            ...
            per_run[run_name] = {
                "generator": generator,
                "round": rnd,
                "level": _level_of_run(leaf_dir, level_override),
                "kernels": n_run,
                "compiled": n_compiled,
                "correct": n_correct,
            }
...
def main() -> None:
    ...
    ap.add_argument("--rounds", nargs="+", type=int, default=None,
                    help="Lint-loop rounds to read (e.g. `0 1 2`, or a single `1` to bin "
                         "one round); omit for the final kernel per sample")
    args = ap.parse_args()
    ...
    build_eval_table(args.runs, args.timing, args.kernelbench_dir, args.out,
                     args.levels, args.rounds)
