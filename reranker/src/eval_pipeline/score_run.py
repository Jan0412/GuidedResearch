...
from reranker.src.eval_pipeline.common import RefArchIndex, git_sha, run_name_of
...
def load_reranker(checkpoint_dir: str, device: str):
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        checkpoint_dir, num_labels=1, torch_dtype=torch.bfloat16,
    )
...
def _run_dir_map(run_dirs: list[str]) -> dict[str, Path]:
    return {run_name_of(d): Path(os.path.abspath(d)) for d in run_dirs}
...
def score_run(
    ...
    run_dirs_map = _run_dir_map(run_dirs)
