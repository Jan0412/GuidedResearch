...
from triton_lint.model import staged_kernel_filename  # noqa: E402
...
def fake_kernelbench(tmp_path, monkeypatch):
    ...
    _clear_caches(runs, triton_lint)
    yield kb, timing
    _clear_caches(runs, triton_lint)


def _clear_caches(runs, triton_lint) -> None:
