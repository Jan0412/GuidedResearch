...
python -m checker check runs/<run_name>/level_6_problem_1_sample_7_kernel.py

# scan a whole run folder (multiprocessing, JSONL output)
python -m checker scan runs/<run_name> --out findings.jsonl --workers 32

# smoke test on the first 500 files, restricted to two checks
python -m checker scan runs/<run_name> --limit 500 --checks F1.2,F2.1 --out /tmp/f.jsonl
...
```python
from checker import analyze_file, analyze_source
