...
def build_skeleton(source: str, path: str = "") -> ModuleModel:
    ...
    model.model_class = resolve_entry_class(tree)
    ...
    if model.entry:
        ...
        model.forward_entry = inherited_forward(tree, model.model_class, model.functions)

    return model


def inherited_forward(
...
def resolve_entry_class(tree: ast.Module) -> str | None:
