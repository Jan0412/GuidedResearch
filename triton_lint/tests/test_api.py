...
import triton_lint
from triton_lint import analyze_file, analyze_source, build_model
...
class TestDegradation:
    def test_a_raising_stage_downgrades_to_partial(self, monkeypatch):
        ...
        monkeypatch.setattr(triton_lint, "analyze_host", boom)
    ...
    def test_a_raising_check_is_skipped_not_fatal(self, monkeypatch):
        from triton_lint import checks
...
class TestReferenceShapeCache:
    def test_repeated_lookups_hit_the_cache(self, fake_kernelbench):
        ...
        first = triton_lint._reference_shapes(1, 2)
        second = triton_lint._reference_shapes(1, 2)

        assert first == second == (((4, 8), "float32"), ((4, 8), "float32"))
        assert triton_lint._reference_shapes.cache_info().hits == 1
